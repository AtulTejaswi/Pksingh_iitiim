...
export default function LessonResourcesPanel({ lessonId, courseId, lessonTitle }: LessonResourcesPanelProps) {
  ...
  return (
    <div className="p-6 border-t border-slate-200 bg-slate-50 space-y-8">
      <p className="text-xs text-slate-500">
        Managing content for: <span className="text-slate-700 font-semibold">{lessonTitle}</span>
      </p>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
            <FileText className="w-4 h-4 text-blue-600" /> Upload PDF / video / image
          </h4>
          <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center relative hover:border-blue-400 transition-all">
            <input
              type="file"
        ...
        <div className="space-y-4">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
            <Link2 className="w-4 h-4 text-sky-600" /> YouTube or web link
          </h4>
          <form onSubmit={handleAddLink} className="space-y-3">
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-900 text-sm outline-none focus:border-sky-500 placeholder:text-slate-400"
            />
            <input
              type="url"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-6 border-t border-slate-200">
        <div className="space-y-4">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Instructor note (text)</h4>
          <form onSubmit={handleAddNote} className="space-y-3">
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-900 text-sm outline-none placeholder:text-slate-400"
            />
            <textarea
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-900 text-sm outline-none resize-none placeholder:text-slate-400"
            />
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Attached content</h4>
          {media.length === 0 && notes.length === 0 && (
            <p className="text-sm text-slate-400 italic">No files, videos, or notes yet.</p>
          )}
          {media.map((item) => (
            <div
              key={item.id}
          ...
          {notes.map((note) => (
            <div
              key={note.id}
              className="flex items-start justify-between gap-2 p-3 rounded-lg border border-slate-200 bg-white text-sm shadow-sm"
            >
              <div className="min-w-0 flex-1">
                <p className="text-slate-800 font-medium">{note.title}</p>
                <p className="text-slate-500 text-xs line-clamp-2 mt-1">{note.content}</p>
              </div>
              <button
                type="button"
                onClick={() =>
