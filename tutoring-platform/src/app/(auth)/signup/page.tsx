...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] flex flex-col items-center justify-center py-12 px-4 relative">
      {/* Mesh gradients */}
      <div className="absolute top-[10%] left-[10%] w-[30%] h-[30%] rounded-full bg-indigo-500/10 blur-[80px] pointer-events-none"></div>
      <div className="absolute bottom-[10%] right-[10%] w-[35%] h-[35%] rounded-full bg-sky-500/5 blur-[90px] pointer-events-none"></div>

      {/* Return to Home link */}
      <Link
        href="/"
        className="absolute top-8 left-8 text-gray-400 hover:text-white flex items-center gap-1.5 text-sm transition-colors"
      >
      ...
      <div className="max-w-md w-full card p-8 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <User className="absolute left-3.5 top-3 w-4 h-4 text-[color:var(--text-muted)]" />
              <input
                type="text"
                {...register('fullName')}
            ...
            {errors.fullName && (
              <p className="text-red-400 text-[10px] font-medium mt-1">{errors.fullName.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <Globe className="absolute left-3.5 top-3.5 w-4 h-4 text-[color:var(--text-muted)]" />
              <select
                {...register('country')}
                className="input-dark pl-10 pr-4 py-2.5 rounded-xl text-sm outline-none appearance-none"
              >
          ...
          <div>
            ...
            <div className="relative">
              <ShieldCheck className="absolute left-3.5 top-3 w-4 h-4 text-[color:var(--text-muted)]" />
              <input
                type="password"
        ...
        <p className="text-center text-xs text-gray-400 mt-6 leading-relaxed">
          ...
          <Link href="/login" className="text-indigo-400 hover:text-white font-semibold transition-colors">
            Sign In
          </Link>
        </p>
      </div>
