...
export default function CoursesPage() {
  ...
  const filteredCourses = courses?.filter((course) => {
    ...
    const matchesPublish = course.isPublished;
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-10 text-left">
        ...
        <p className="text-gray-400 text-sm">Select your exam target or filter by subject to begin your training.</p>
      </div>
      ...
      {isLoading ? (
        <div className="grid md:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] h-80 animate-pulse flex flex-col p-6 justify-between">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {filteredCourses.map((course) => (
            ...
            >
              {/* Course Card Banner with specialized color gradients based on subject */}
              <div className={`h-40 relative p-6 flex flex-col justify-between border-b border-[rgba(255,255,255,0.06)] bg-gradient-to-br ${
